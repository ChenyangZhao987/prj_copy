...
(* MARK_DEBUG="true" *)reg			[15:0]				data_out;
...
always@(posedge bram_clk_b)
	if(r_rst)begin
		...
		data_reg <= data_flag ? {data_reg[47:0],data_out} : data_reg;
	end 
...
always@(posedge bram_clk_b)
	if(r_rst)begin
		...
		reg_cnt <=  reg_cnt == 'd3 ? 'd0 : reg_cnt + 1'b1;
		din_reg <= 	reg_cnt == 'd3 ? {data_reg[47:0],data_out}: din_reg;
	end else begin
		reg_cnt <= reg_cnt;
		din_reg <= din_reg;
	end
...
always@(posedge bram_clk_b)
	if(r_rst)begin	
		...
		en_s1 <= data_flag && reg_cnt == 'd3 ? 1'b1 : 1'b0;
		en_s  <= en_s1;
	end
...
*******************************************************************************/  
wire		[94:0]		probe0;
(* MARK_DEBUG="true" *)wire					sda1,sclk1;
assign	sda1 = Ssda;
assign sclk1 = Ssclk;	
	ila_0 ila_ins (
