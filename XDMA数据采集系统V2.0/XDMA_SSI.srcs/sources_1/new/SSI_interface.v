...
//					1、接口适配：通过 12 位地址总线、64 位数据总线与外部主控（如 CPU）通信，支持寄存器读写操作；
//					SSI 参数配置：
//						1、控制 SSI 时钟频率（ssi_sda_ctrl），对应 SSI_interface 的 Sda_ctrl 信号；
//						1、控制 SSI 片选使能（ssi_cs_en），对应 SSI_interface 的 cs_en 信号；
//						1、配置 SSI 接收数据位宽（ssi_rx_size），对应 SSI_interface 的 RX_SIZE 参数；
//              
// Dependencies: 
// 
// Revision:
...
module SSI_interface#(
	...
	input						Phy_Clk,
	input						Phy_Rst,
	input						Sda_ctrl,//发送时钟周期0：12.5MHz  1： 5MHz	
	output	reg					Ssclk,
	output	reg					Cs_n,
	input						cs_en,
	input						Ssda,
...
*******************************************************************************/
wire	[10:0]		CNT_MAX	;
assign CNT_MAX = Sda_ctrl == 0 ? 100 : 200 ;//输出时钟： 0：10MHz  1： 5MHz
