...
SSI_interface#(
		.CLK_T      (CLK_T					),  //时钟周期
		.DATA_TIME	(20_000					)	//20_000ns=20us
	)SSI_interface_inst(
		.Phy_Clk    (Clk					),
		.Phy_Rst    (r_rst					),
...
*******************************************************************************/
reg				[13:0]		addr_sel;
(* MARK_DEBUG="true" *)reg				[27:0]		addr_s;

always@(*)begin
	case(single_sample)
		17'h0	:	addr_sel = 14'h80;		//17'h800	
		17'h1	:	addr_sel = 14'h100;		//17'h1000	
		17'h2	:	addr_sel = 14'h200;		//17'h2000	
		17'h3	:	addr_sel = 14'h400;		//17'h4000	
		17'h4	:	addr_sel = 14'h800;		//17'h8000	
...
(* MARK_DEBUG="true" *)reg			[2:0]			addr_delay;
...
always@(posedge Clk)
	if(r_rst)begin
		...
		ch_wen <= data_flag  && data_acq_en ? 1'b1 : 1'b0;
	end
...
always@(posedge Clk)
	if(r_rst)begin
		en_s1 		<= 'd0;
		addr_delay 	<= 'd0;
	end else case(addr_delay)
		0		:	begin
						addr_delay <= ch_wen ? 'd1 : addr_delay;
						en_s1 <= 'd0;
					end
		1,2,3,4	: 	begin 
						addr_delay <= addr_delay == 'd4 ? 'd0 : addr_delay + 1'b1;
						en_s1 <= addr_delay == 'd4 ? 'd1 :'d0;
					end
		default	:	begin	en_s1 <= 'd0;addr_delay 	<= 'd0;	end
	endcase
...
always@(posedge Clk)
	if(r_rst)begin
		...
		case(irq_state0)
			0	:	begin
						irq_state0 <= (addr_s == ('h800 + addr_sel))  && ~req_1 ?  'd1 : 'd0;//&& en_s1
						req_0	<= 1'b0;
					end 
			1	:	begin
...
always@(posedge Clk)
	if(r_rst)begin
		...
		case(irq_state1)
			0	:	begin
						irq_state1 <= (addr_s == addr_sel)  && ~req_0 ?  'd1 : 'd0;//&& en_s1
						req_1	<= 1'b0;
					end 
			1	:	begin
