...
always@(*)
	if(r_rst)begin
		...
		0	:	CNT_MAX <= 500; 	//2MHz
		1	:	CNT_MAX <= 800; 	//1.25MHz
		2	:	CNT_MAX <= 1000; 	//1MHz
		3	:	CNT_MAX <= 1250;	//800KHz
		4	:	CNT_MAX <= 2;	//1MHz
		default:CNT_MAX <= 800; //1.25MHz 
	endcase
...
reg			[15:0]		rev_cnt;
(* MARK_DEBUG="true" *)reg			[7:0]		bit_cnt;
...
always@(posedge Phy_Clk)
	if(r_rst)begin
		...
		case(rx_state)
			...
			START	:	rx_state <= neg_scl ? RX_RDAT	: START;//rev_cnt == (STAT_T - 1)
